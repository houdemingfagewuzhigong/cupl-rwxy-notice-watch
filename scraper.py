#!/usr/bin/env python3
"""Fetch CUPL School of Humanities notices."""
...
BASE_URL = "https://rwxy.cupl.edu.cn"
USER_AGENT = "Mozilla/5.0 (compatible; cupl-rwxy-notice-watch/1.0; +https://github.com/houdemingfagewuzhigong)"
DATA_DIR = Path("data")

SECTIONS = [
    {"name": "通知公告", "path": "index/tzgg.htm"},
    {"name": "学院动态", "path": "index/xydt.htm"},
]
...
def normalize_date(text: str) -> str:
    ...
    match = re.search(r"(\d{1,2})[-/.](\d{1,2})\s+(20\d{2})", text or "")
    if match:
...
def title_from_detail(url: str, fallback: str) -> str:
    try:
        detail_html = open_with_challenge(url)
    except Exception:
        return fallback
    match = re.search(r"<title>(.*?)</title>", detail_html, re.S | re.I)
    if not match:
        return fallback
    title = clean(match.group(1))
    title = re.sub(r"\s*[-_—|]\s*中国政法大学人文学院\s*$", "", title).strip()
...
def parse_list(html_text: str, section: str, source_url: str) -> list[Notice]:
    seen_at = now_iso()
    items: list[Notice] = []
    item_pattern = re.compile(r"<LI[^>]*>(.*?)</LI>", re.S | re.I)
    for block_html in item_pattern.findall(html_text):
        href_match = re.search(r'<A[^>]+href="([^"]*info/\d+/\d+\.htm)"[^>]*>(.*?)</A>', block_html, re.S | re.I)
        if not href_match:
            continue
        href, title_html = href_match.groups()
        url = absolute_url(href, source_url)
        date_match = re.search(r'class="you"[^>]*>(.*?)</SPAN>', block_html, re.S | re.I)
        date = normalize_date(date_match.group(1) if date_match else clean(block_html))
        title = clean(title_html)
        title = title_from_detail(url, title)
        summary = ""
        items.append(
            Notice(
                id=notice_id(url),
...
def save(notices: list[Notice]) -> None:
    ...
    meta = {
        "site": "中国政法大学人文学院",
        "base_url": BASE_URL,
        "notice_url": BASE_URL + "/index/tzgg.htm",
        "updated_at": seen_at,
